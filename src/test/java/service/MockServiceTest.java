...
public class MockServiceTest {
    ...
    //1-1.어떤 position을 set 해도 미드를 리턴하도록 테스트
    @Test
    public void shouldReturnValueWhenAnythingPosition(){
        Champion champion = mock(Champion.class);
        champion.setName("탑");
    ...
    @Test
    public void shouldAtLeastThreeTimesForChampiongetName(){
        Champion champion = mock(Champion.class);
        champion.setName("조이");
        champion.setPosition("미드");
        champion.setHasSkinCount(3);
    ...
    @Test
    public void checkFindlissandraandPositionTop(){
    ...
    @Test
    public void checkFindNameandSkinCount(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("루시안", "바텀", 5));
        assertThat(mockService.findByName("애쉬").getHasSkinCount(),is(5));
        verify(mockRepository,times(1)).findByName(anyString());
    }
    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void 검색호출횟수검증(){
        List<Champion>champions = new ArrayList<>();
        champions.add(new Champion("루시안", "바텀", 5));
        champions.add(new Champion("리산드라", "미드", 4));
        champions.add(new Champion("탐켄치", "서폿", 5));
